...
namespace clientForQuestions2._0
{
    public partial class chaptersQuestionsMenu : Form
    {
        ...
        private Dictionary<string, List<string>> chapter2topic = new Dictionary<string, List<string>>
        {
            {
        ...
        {
            { "חשיבה כמותית", new List<int> { 45, 47, 48, 49, 50, 55, 63, 64, 65, 70, 71, 72, 73, 74, 78, 79, 80, 81, 82, 87, 88, 89, 90, 91, 92, 93, 94, 95, 96, 97, 127, 128, 129, 130, 132, 133, 134, 135 } },
            // only collections who have 6 questions
            { "חשיבה מילולית", new List<int> { 39, 40, 56, 57, 58, 60, 61, 62, 75, 77, 98, 119, 120, 123, 124, 125, 126 } },
            { "אנגלית", new List<int> { 41, 42, 43, 44, 46, 51, 52, 53, 54, 59, 66, 67, 68, 69, 83, 84, 85, 86, 99, 100, 101, 102, 103, 104, 105, 106, 107, 108, 109, 110, 111, 112, 113, 114, 115, 116, 117, 136 } }
        };
        ...
        Dictionary<int, List<int>> colId2qIds = new Dictionary<int, List<int>>()
{
    ...
    { 136, new List<int> { 11157, 11158, 11159, 11160, 11161 } }
        ...
        private void sendChapter1_Questions()
        {
            ...
            // get הבנה והסקה
            List<dbQuestionParmeters> havana_qs = sqlDb.get_n_questions_from_arr_of_categorys(11, chapter2topic["הבנה והסקה"]); // qs of normal qs
            havana_qs.Sort((x, y) => ((float)x.json_content["difficulty_level"]).CompareTo((float)y.json_content["difficulty_level"]));
            // get קטע קריאה
            List<int> colIds = title2colIds["חשיבה מילולית"]; // get all ids of קטע קריאה
            ...
            foreach (int col_qID in colId2qIds[col_id])
            {
        ...
        {
            List<String> topics = chapter2topic["חשיבה כמותית"];

            List<int> colIds = title2colIds["חשיבה כמותית"]; // get all ids of tarshim
            int col_id = colIds[random.Next(colIds.Count)]; // choose rand collection of the category
        ...
        private void sendChapter3_Questions()
        {
            ...
            // get קטע קריאה
            List<int> colIds = title2colIds["אנגלית"]; // get all ids of קטע קריאה
            ...
            foreach (int col_qID in colId2qIds[col_id1])
            {
                col_qs.Add(sqlDb.get_question_based_on_id(col_qID));
            }
            foreach (int col_qID in colId2qIds[col_id2]) // TODO the easier text first
            {
                col_qs.Add(sqlDb.get_question_based_on_id(col_qID));
            }
